...
/**
 * @title AutoPenaltyEscrowV3
 * @notice Escrows corporate penalties, auto‑pays community electricity fund,
 *         distributes micro‑rewards to verified households,
 *         and logs emotional APR + damay clause tags.
 */
contract AutoPenaltyEscrowV3 {
    ...
    event HouseholdReward(address indexed wallet, uint256 amount, string purpose);
    ...
    function logPenalty(
        ...
        _autoPayCommunityAndHouseholds(id, emotionalAPR, damayClause);
    }

    function _autoPayCommunityAndHouseholds(
        address id,
        ...
        // 50% hati-hati sa verified households
        ...
        for (uint256 i = 0; i < householdIndex.length; i++) {
            if (households[householdIndex[i]].verified && perHousehold > 0) {
                ...
                emit HouseholdReward(householdIndex[i], perHousehold, "Penalty micro‑disbursement");
            }
        }

        payoutCount++;
        payoutLedger[payoutCount] = PayoutRecord(payout, "Electric utility + household rewards", emotionalAPR, damayClause);
