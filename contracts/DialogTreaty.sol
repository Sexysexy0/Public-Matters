...
contract DialogTreaty {
    ...
    mapping(uint256 => DialogRule) public treaties;
    ...
    function declareDialogRule(string calldata principle, string calldata description) external onlyOverseer {
        treatyCount++;
        treaties[treatyCount] = DialogRule(treatyCount, principle, description, block.timestamp);
        emit DialogRuleDeclared(treatyCount, principle, description);
    }
}
