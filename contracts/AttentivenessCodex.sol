pragma solidity ^0.8.20;

contract AttentivenessCodex {
    address public overseer;
    uint256 public codexCount;

    struct AttentivenessRule {
        uint256 id;
        string principle;   // Focus, Care, Mindful Observation
        string description;
        uint256 timestamp;
    }

    mapping(uint256 => AttentivenessRule) public rules;
    event AttentivenessRuleDeclared(uint256 indexed id, string principle, string description);

    constructor(address _overseer) { overseer = _overseer; }
    modifier onlyOverseer() { require(msg.sender == overseer, "Not authorized"); _; }

    function declareAttentivenessRule(string calldata principle, string calldata description) external onlyOverseer {
        codexCount++;
        rules[codexCount] = AttentivenessRule(codexCount, principle, description, block.timestamp);
        emit AttentivenessRuleDeclared(codexCount, principle, description);
    }
}
