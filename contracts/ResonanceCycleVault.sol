...
contract ResonanceCycleVault {
    ...
    event CycleLogged(address indexed steward, string arc, string safeguard);
    event CycleSafeguarded(uint256 indexed id, address verifier);

    function logCycle(string memory _arc, string memory _safeguard) public {
        cycles.push(Cycle(msg.sender, _arc, _safeguard, block.timestamp, false));
        emit CycleLogged(msg.sender, _arc, _safeguard);
    }
