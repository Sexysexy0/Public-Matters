...
contract InquiryCodex {
    address public overseer;
    uint256 public codexCount;

    struct InquiryRule {
        uint256 id;
        string principle;   // Questioning, Investigation, Truth-Seeking
        string description;
        uint256 timestamp;
    }
    ...
    function declareInquiryRule(string calldata principle, string calldata description) external onlyOverseer {
        codexCount++;
        rules[codexCount] = InquiryRule(codexCount, principle, description, block.timestamp);
        emit InquiryRuleDeclared(codexCount, principle, description);
    }
}
