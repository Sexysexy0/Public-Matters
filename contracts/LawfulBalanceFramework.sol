...
/// @title Lawful Balance Framework
/// @notice Encodes lawful balance safeguards.
/// @dev Complements DignityTreaty, JusticeMandala, and IntegrityCharter.

contract LawfulBalanceFramework {
    ...
    enum BalanceRule {
        BalanceIsConstitutional,
        LawAnchored,
        EquilibriumProtected,
        ExcessSuppressionSafeguarded,
        PublicBenefitPriority,
        MandatoryCouncilOversight,
        TransparencyInBalanceSystems
    }
    ...
    event RuleDeclared(uint256 indexed id, BalanceRule ruleType);
    ...
    function _declareDefaultRules() internal {
        _declare(BalanceRule.BalanceIsConstitutional, "Balance is constitutional; denial prohibited.");
        _declare(BalanceRule.LawAnchored, "Law anchored; distortion prohibited.");
        _declare(BalanceRule.EquilibriumProtected, "Equilibrium protected; collapse prohibited.");
        _declare(BalanceRule.ExcessSuppressionSafeguarded, "Excess suppression safeguarded; enforcement prohibited.");
        _declare(BalanceRule.PublicBenefitPriority, "Public benefit overrides elite gain.");
        _declare(BalanceRule.MandatoryCouncilOversight, "Council oversight required for balance enforcement.");
        _declare(BalanceRule.TransparencyInBalanceSystems, "Balance systems must be transparent.");
    }

    function _declare(BalanceRule ruleType, string memory description) internal {
        frameworkCount++;
