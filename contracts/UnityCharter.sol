...
/// @title Unity Charter
/// @notice Firewall for unity and collective strength.
/// @dev Complements CompassionFramework, CommunalHarmonyTreaty, and HumanSolidarityTreaty.

contract UnityCharter {
    ...
    enum UnityRule {
        UnityIsConstitutional,
        CollectiveStrengthAnchored,
        CooperationProtected,
        SolidaritySafeguarded,
        PublicBenefitPriority,
        MandatoryCouncilOversight,
        TransparencyInUnitySystems
    }
    ...
    event RuleDeclared(uint256 indexed id, UnityRule ruleType);
    ...
    function _declareDefaultRules() internal {
        _declare(UnityRule.UnityIsConstitutional, "Unity is constitutional; denial prohibited.");
        _declare(UnityRule.CollectiveStrengthAnchored, "Collective strength is anchored; suppression prohibited.");
        _declare(UnityRule.CooperationProtected, "Cooperation is protected; blocking prohibited.");
        _declare(UnityRule.SolidaritySafeguarded, "Solidarity safeguarded; erasure prohibited.");
        _declare(UnityRule.PublicBenefitPriority, "Public benefit overrides elite gain.");
        _declare(UnityRule.MandatoryCouncilOversight, "Council oversight required for unity enforcement.");
        _declare(UnityRule.TransparencyInUnitySystems, "Unity systems must be transparent.");
    }

    function _declare(UnityRule ruleType, string memory description) internal {
        charterCount++;
