...
/// @title Supplemental Filings Treaty
/// @notice Encodes supplemental filings safeguard.
/// @dev Complements DecisionFormatFramework, CaseLawCodification, and AppealsMandala.

contract SupplementalFilingsTreaty {
    ...
    enum FilingRule {
        SupplementalFilingsRegulated,
        CodificationRequired,
        AbuseSuppressed,
        UniformityEnforced,
        TransparencyInFilingSystems,
        PublicBenefitPriority
    }

    enum ViolationType {
        FilingAbuse,
        ExcessiveSubmissions,
        Misapplication,
        CouncilBypass,
    ...
    event RuleDeclared(uint256 indexed id, FilingRule ruleType);
    ...
    function _declareDefaultRules() internal {
        _declare(FilingRule.SupplementalFilingsRegulated, "Supplemental filings regulated; uniformity required.");
        _declare(FilingRule.CodificationRequired, "Codification required; case law consistency enforced.");
        _declare(FilingRule.AbuseSuppressed, "Abuse suppressed; excessive filings blocked.");
        _declare(FilingRule.UniformityEnforced, "Uniformity enforced; divergence prohibited.");
        _declare(FilingRule.TransparencyInFilingSystems, "Filing systems must be transparent.");
        _declare(FilingRule.PublicBenefitPriority, "Public benefit overrides elite gain.");
    }

    function _declare(FilingRule ruleType, string memory description) internal {
        treatyCount++;
