...
/// @notice Encodes peace commitments into treaty law, prohibiting secret wars and enforcing transparency.
/// @dev Complements WarPowersTransparency, JusticeOversightCovenant, and GlobalTradeTreaty.

contract InternationalPeaceTreaty {
    ...
    enum PeaceRule {
        NoSecretWars,
        NoUnprovokedAggression,
        MandatoryTransparency,
        MandatoryCouncilOversight,
        PublicBenefitPriority,
        PeaceIntegrityIsConstitutional,
    ...
    event RuleDeclared(uint256 indexed id, PeaceRule ruleType);
    ...
    function _declareDefaultRules() internal {
        _declare(PeaceRule.NoSecretWars, "Secret wars are prohibited.");
        _declare(PeaceRule.NoUnprovokedAggression, "Unprovoked aggression is prohibited.");
        _declare(PeaceRule.MandatoryTransparency, "Peace commitments must be transparent.");
        _declare(PeaceRule.MandatoryCouncilOversight, "Council oversight required for treaty enforcement.");
        _declare(PeaceRule.PublicBenefitPriority, "Public benefit overrides political gain.");
        _declare(PeaceRule.PeaceIntegrityIsConstitutional, "Peace integrity is constitutional.");
        _declare(PeaceRule.TreatyBindingAcrossMembers, "Treaty rules are binding across all members.");
    }

    function _declare(PeaceRule ruleType, string memory description) internal {
        treatyCount++;
