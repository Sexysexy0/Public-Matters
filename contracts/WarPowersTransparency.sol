...
/// @title War Powers Transparency Covenant
/// @notice Firewall for executive war powers and military actions.
/// @dev Complements CitizenLoyaltyCharter, DefenseTransparency, and NationalSecurityCovenant.

contract WarPowersTransparency {
    ...
    enum WarRule {
        ...
        NoSecretWarDeclarations,
        NoUnauthorizedMilitaryStrikes,
        MandatoryCouncilOversight,
        PublicBenefitPriority,
        TransparencyOverSecrecy,
        WarPowersIntegrityIsConstitutional
    }
    ...
    event RuleDeclared(uint256 indexed id, WarRule ruleType);
    ...
    function _declareDefaultRules() internal {
        _declare(WarRule.MandatoryPublicLedgerEntry, "All war powers actions must be logged in the InnovationLedger.");
        _declare(WarRule.NoSecretWarDeclarations, "Secret war declarations are prohibited.");
        _declare(WarRule.NoUnauthorizedMilitaryStrikes, "Unauthorized military strikes are prohibited.");
        _declare(WarRule.MandatoryCouncilOversight, "Council oversight required for war powers actions.");
        _declare(WarRule.PublicBenefitPriority, "Public benefit overrides private or political gain.");
        _declare(WarRule.TransparencyOverSecrecy, "Transparency overrides secrecy in war powers.");
        _declare(WarRule.WarPowersIntegrityIsConstitutional, "War powers integrity is constitutional.");
    }

    function _declare(WarRule ruleType, string memory description) internal {
        ruleCount++;
