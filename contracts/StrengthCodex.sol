...
contract StrengthCodex {
    address public overseer;
    uint256 public codexCount;

    struct StrengthRule {
        uint256 id;
        string principle;   // Strength, Fear, Mentor, Sacrifice
        string description;
        uint256 timestamp;
    }
    ...
    function declareStrengthRule(string calldata principle, string calldata description) external onlyOverseer {
        codexCount++;
        rules[codexCount] = StrengthRule(codexCount, principle, description, block.timestamp);
        emit StrengthRuleDeclared(codexCount, principle, description);
    }
}
