...
/// @notice Encodes visibility safeguards.
/// @dev Complements ResponsibilityTreaty, TransparencyMandala, and AccountabilityTreaty.

contract VisibilityFramework {
    ...
    enum VisibilityRule {
        VisibilityIsConstitutional,
        OpennessAnchored,
        ConcealmentProhibited,
        HiddenRiskSuppressed,
        PublicBenefitPriority,
        MandatoryCouncilOversight,
        TransparencyInVisibilitySystems
    }
    ...
    event RuleDeclared(uint256 indexed id, VisibilityRule ruleType);
    ...
    function _declareDefaultRules() internal {
        _declare(VisibilityRule.VisibilityIsConstitutional, "Visibility is constitutional; denial prohibited.");
        _declare(VisibilityRule.OpennessAnchored, "Openness anchored; failure prohibited.");
        _declare(VisibilityRule.ConcealmentProhibited, "Concealment prohibited; violation blocked.");
        _declare(VisibilityRule.HiddenRiskSuppressed, "Hidden risks suppressed; breach prohibited.");
        _declare(VisibilityRule.PublicBenefitPriority, "Public benefit overrides elite gain.");
        _declare(VisibilityRule.MandatoryCouncilOversight, "Council oversight required for visibility enforcement.");
        _declare(VisibilityRule.TransparencyInVisibilitySystems, "Visibility systems must be transparent.");
    }

    function _declare(VisibilityRule ruleType, string memory description) internal {
        frameworkCount++;
