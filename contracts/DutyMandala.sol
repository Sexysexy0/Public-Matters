...
/// @title Duty Mandala
/// @notice Weaves duty into mandala resonance.
/// @dev Complements ResponsibilityTreaty, VisibilityFramework, and AccountabilityTreaty.

contract DutyMandala {
    ...
    event RuleDeclared(uint256 indexed id, DutyRule ruleType);
    ...
    function _declareDefaultRules() internal {
        _declare(DutyRule.DutyIsConstitutional, "Duty is constitutional; denial prohibited.");
        _declare(DutyRule.ResponsibilityAnchored, "Responsibility anchored; failure prohibited.");
        _declare(DutyRule.IrresponsibilityProhibited, "Irresponsibility prohibited; violation blocked.");
        _declare(DutyRule.NeglectBlocked, "Neglect blocked; breach prohibited.");
        _declare(DutyRule.PublicBenefitPriority, "Public benefit overrides elite gain.");
        _declare(DutyRule.MandatoryCouncilOversight, "Council oversight required for duty enforcement.");
        _declare(DutyRule.TransparencyInDutySystems, "Duty systems must be transparent.");
    }

    function _declare(DutyRule ruleType, string memory description) internal {
        mandalaCount++;
