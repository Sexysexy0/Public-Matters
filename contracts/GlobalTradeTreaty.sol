...
/// @title Global Trade Treaty
/// @notice Unifies trade fairness covenants into international treaty law.
/// @dev Integrates TradeFairnessCharter, SupplyChainTransparency, ResourceSecurityCovenant, and EnergyResilienceCovenant.

contract GlobalTradeTreaty {
    ...
    event RuleDeclared(uint256 indexed id, TreatyRule ruleType);
    ...
    function _declareDefaultRules() internal {
        _declare(TreatyRule.NoWeaponizedTrade, "Trade cannot be weaponized for geopolitical leverage.");
        _declare(TreatyRule.NoExploitativeSanctions, "Sanctions cannot exploit or punish citizens unfairly.");
        _declare(TreatyRule.MandatoryTransparency, "Trade agreements must be transparent.");
        _declare(TreatyRule.MandatoryCouncilOversight, "Council oversight required for treaty enforcement.");
        _declare(TreatyRule.PublicBenefitPriority, "Public benefit overrides private gain.");
        _declare(TreatyRule.FairAccessIsConstitutional, "Fair access to trade is constitutional.");
        _declare(TreatyRule.TreatyBindingAcrossMembers, "Treaty rules are binding across all members.");
    }

    function _declare(TreatyRule ruleType, string memory description) internal {
        treatyCount++;
