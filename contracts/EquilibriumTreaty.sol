...
/// @title Equilibrium Treaty
/// @notice Unifies equilibrium commitments into treaty law.
/// @dev Complements HopeMandala, BalanceFramework, and JusticeEquilibrium.

contract EquilibriumTreaty {
    address public guardian;
    uint256 public treatyCount;
    uint256 public violationCount;
    uint256 public memberCount;

    enum EquilibriumRule {
        EquilibriumIsConstitutional,
        FairnessAnchored,
        StabilityProtected,
        HarmonySafeguarded,
    ...
    event RuleDeclared(uint256 indexed id, EquilibriumRule ruleType);
    ...
    function _declareDefaultRules() internal {
        _declare(EquilibriumRule.EquilibriumIsConstitutional, "Equilibrium is constitutional; denial prohibited.");
        _declare(EquilibriumRule.FairnessAnchored, "Fairness is anchored; suppression prohibited.");
        _declare(EquilibriumRule.StabilityProtected, "Stability is protected; blocking prohibited.");
        _declare(EquilibriumRule.HarmonySafeguarded, "Harmony safeguarded; erasure prohibited.");
        _declare(EquilibriumRule.PublicBenefitPriority, "Public benefit overrides elite gain.");
        _declare(EquilibriumRule.MandatoryCouncilOversight, "Council oversight required for equilibrium enforcement.");
        _declare(EquilibriumRule.TreatyBindingAcrossMembers, "Treaty rules binding across all members.");
    }

    function _declare(EquilibriumRule ruleType, string memory description) internal {
        treatyCount++;
